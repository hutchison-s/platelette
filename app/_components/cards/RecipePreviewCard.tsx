...
function RecipePreviewCard({recipe}: {recipe: Recipe}) {
    return (
        <Card className="grid gap-2">
            <Link href={'/recipes/'+encodeURIComponent(recipe.title).replaceAll('%20', '_')}>
                <img src={recipe.photo} alt='Recipe photo' width={200} className="w-full h-32 object-cover rounded-std md:h-60"/>
            </Link>
            <div className="w-full flex gap-2 justify-between items-center">
                <h3 className="shrink font-heading font-bold text-foreground text-xl md:text-2xl"><Link href={'/recipes/'+encodeURIComponent(recipe.title).replaceAll('%20', '_')}>{recipe.title}</Link></h3>
